#!/usr/bin/env python3
"""
DivorceGPT NJ - Certification of Insurance Coverage
Pursuant to R. 5:4-2(f)

SHORT FORM — for cases where the only relief sought is dissolution.
R. 5:4-2(f) provides: "If, however, the only relief sought is dissolution
of the marriage or civil union, or a termination of a domestic partnership,
...the parties shall annex to their pleadings, in lieu of the required
insurance affidavit, an affidavit so stating."

This form certifies that the only relief sought is dissolution, therefore
the full insurance listing is not required. It also certifies that no
insurance coverage has been canceled or modified within 90 days preceding
the date of this certification, and that existing coverage will be
maintained pending further order of the Court.

Same caption/signature format as other NJ forms.
"""
...
def generate_nj_insurance(data, output_path):
    """
    Generate NJ Certification of Insurance Coverage (short form).
    ...
    # =====================================================================
    c.setFont(FONT, FONT_SIZE)
    c.drawString(MARGIN_LEFT, y, f"{p_name}, being of full age, hereby certifies as follows:")
    y -= DOUBLE_SPACE * 1.2
    ...
    y = draw_wrapped_justified(c,
        "I am the Plaintiff in the above captioned matter and I make this "
        "Certification pursuant to R. 5:4-2(f).",
    ...
    y = draw_wrapped_justified(c,
        ...
        "R. 5:4-2(f) is not required, and this Certification is submitted in lieu "
        "thereof.",
        para_x, y, para_width, line_height=DOUBLE_SPACE)
    y -= DOUBLE_SPACE * 0.1
    ...
    y = draw_wrapped_justified(c,
        "I understand that all existing insurance coverage shall be maintained "
        "pending further Order of the Court, pursuant to R. 5:4-2(f).",
        para_x, y, para_width, line_height=DOUBLE_SPACE)
    y -= DOUBLE_SPACE * 0.5
...
if __name__ == "__main__":
    ...
    output = generate_nj_insurance(test_data, "/tmp/test_nj_insurance.pdf")
